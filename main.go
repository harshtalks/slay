...
var (
	...
	descStyle = lipgloss.NewStyle().MarginTop(1).Foreground(lipgloss.Color("5"))

	infoStyle = lipgloss.NewStyle().
			BorderStyle(lipgloss.NormalBorder()).
			BorderTop(true).
			BorderForeground(subtle).Foreground(special)
...
func main() {
	...
	mwheel :=
		key.NewBinding(
			key.WithKeys("mouse-wheel"),
			key.WithHelp("mouse-wheel", "up/down"),
		)
...
func getProcesses() []list.Item {
	...
	for i, proc := range procs {
		...
		titleStr := fmt.Sprintf("%s :%s (%s)", command, port, pid)
		descStr := fmt.Sprintf("User: %s, Port: %s, Pid: %s, Command: %s", user, port, pid, command)
...
func killPort(pid string) {
	...
	if err != nil {
		log.Error("Oops!!, couldn't kill, take the L with - ", err)
	}
...
func confirmationView(m model) string {
	...
	if m.activeButton == "yes" {
		okButton = activeButtonStyle.Render("Fuck Yeah!!")
		cancelButton = buttonStyle.
			Render("Hold up, lemme think")
	} else {
		okButton = buttonStyle.Render("Bruhh!!")
		cancelButton = activeButtonStyle.
			Render("thinking of not..")
	}

	qStr := fmt.Sprintf("sure to slay this process - %s?", m.selectedPort)
...
func initTitle() string {
	...
	desc := lipgloss.JoinVertical(lipgloss.Left,
		descStyle.Render("Don't Hesitate, kill these processes mami"),
		infoStyle.Render("Them processes better not fcuk wid ya..."),
	)
